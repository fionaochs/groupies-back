        ...
        await Promise.all(
            saved.map(favorite => {
                return client.query(`
                    ...
                    RETURNING *;
                `,
                [1, favorite.name, favorite.images, favorite.classifications.genre.name, favorite.dates.start.localDate, favorite.url, favorite.venues.city.name, favorite.venues.state.name, favorite.priceRanges.min, favorite.priceRanges.max, favorite.venues.location.latitude, favorite.venues.location.longitude, favorite.id]);
            })
        );
